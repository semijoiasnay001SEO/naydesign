const produtos = [
  {id:1, nome:"Choker fina 2mm", preco:79.90, categoria:"Chokers", cores:["Dourado"], imagem:"imagens/IMG-20260909-WA0066.jpg"},
  {id:2, nome:"Choker fita 2mm", preco:79.90, categoria:"Chokers", cores:["Dourado"], imagem:"imagens/IMG-20260909-WA0088.jpg"},
  ...
  {id:55, nome:"Brinco argolinha coração", preco:39.90, categoria:"Brincos", cores:["Dourado"], imagem:"imagens/IMG-20260909-WA0112.jpg"},
  {id:56, nome:"Brinco argola coração vazado", preco:69.90, categoria:"Brincos", cores:["Dourado", "Prateado"], imagem:"imagens/IMG-20260909-WA0121.jpg"},
  {id:57, nome:"Brinco coração cravejado", preco:59.90, categoria:"Brincos", cores:["Dourado", "Ródio"], imagem:"imagens/IMG-20260909-WA0122.jpg"},
  {id:58, nome:"Brinco coração cravejado vazado", preco:49.90, categoria:"Brincos", cores:["Dourado", "Prateado"], imagem:"imagens/IMG-20260909-WA0123.jpg"},
  ...
  {id:60, nome:"Trio de brincos florzinha", preco:109.90, categoria:"Brincos", cores:["Prateado"], imagem:"imagens/IMG-20260909-WA0131(1).jpg"},
  ...
  {id:64, nome:"Piercing fake zircônia 2mm", preco:35.90, categoria:"Piercings", cores:["Prateado"], imagem:"imagens/IMG-20260909-WA0131(1).jpg"},
...
function abrirWhatsApp(produto) {
  ...
  window.open("https://wa.me/5519992763019?text=" + encodeURIComponent(msg), "_blank");
}
function adicionarProdutoAoCarrinho(produto) {
  if (typeof window.adicionarAoCarrinho === "function") return window.adicionarAoCarrinho(produto);
  document.dispatchEvent(new CustomEvent("nay:adicionar-carrinho", {detail:produto}));
}
function renderizarProdutos(lista) {
  const container=document.getElementById("produtos"); if(!container)return; container.innerHTML="";
  lista.forEach(function(produto){
    const card=document.createElement("article"); card.className="produto-card";
    const topo=document.createElement("div"); topo.className="produto-marca";
    const logo=document.createElement("img"); logo.src="logo.jpg"; logo.alt="Nayara Oliveira Semi Joias"; topo.appendChild(logo);
    const imagem=document.createElement("div"); imagem.className="produto-imagem";
    const img=document.createElement("img"); img.src=produto.imagem; img.alt=produto.nome; img.loading="lazy";
    img.onerror=function(){imagem.innerHTML='<span class="sem-imagem">Foto da peça</span>';}; imagem.appendChild(img);
    ...
    const preco=document.createElement("strong"); preco.className="produto-preco"; preco.textContent=formatarPreco(produto.preco);
    const badge=document.createElement("span"); badge.className="produto-badge"; badge.textContent="NOVA COLEÇÃO";
    const banho=document.createElement("div"); banho.className="produto-banho"; banho.textContent="DISPONÍVEL EM "+produto.cores.join(" OU ");
    const detalhes=document.createElement("div"); detalhes.className="produto-detalhes"; detalhes.innerHTML='<span>♢ &nbsp;Alta durabilidade</span><span>✧ &nbsp;Brilho intenso</span><span>♡ &nbsp;Design delicado</span><span>◇ &nbsp;Qualidade garantida</span>';
    const acoes=document.createElement("div"); acoes.className="produto-acoes";
    const bc=document.createElement("button"); bc.className="btn-carrinho"; bc.type="button"; bc.textContent="Adicionar ao carrinho"; bc.onclick=function(){adicionarProdutoAoCarrinho(produto);};
    const bw=document.createElement("button"); bw.className="btn-whatsapp"; bw.type="button"; bw.textContent="Comprar pelo WhatsApp"; bw.onclick=function(){abrirWhatsApp(produto);};
    acoes.append(bc,bw); info.append(nome,preco,badge,banho,detalhes,acoes); card.append(topo,imagem,info); container.appendChild(card);
  });
}
